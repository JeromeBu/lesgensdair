class OrdersController < ApplicationController
  ...
  def create
    ...
    @order  = Order.create(product_sku: @product.sku, product_name: @product.name, amount: amount_to_pay, state: 'en attente', first_name: order_params[:first_name].capitalize, last_name: order_params[:last_name].capitalize, email: order_params[:email], video: order_params[:video])
    authorize @order
    if @order.save
